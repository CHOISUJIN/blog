...
export default function PostCard({ post }: { post: Post }) {
  return (
    <Link key={post.slug} href={`/${post.slug}`}>
      <article className="w-full flex flex-col sm:flex-row gap-6 md:gap-10 transition-all group border rounded-lg p-6 hover:shadow-lg hover:dark:shadow-gray-300">
        {/* 텍스트 섹션 */}
        ...
        <div className="w-full sm:w-[300px] shrink-0 order-1 sm:order-2">
          <FixedSizeImgWithPlaceholder
            className="w-full aspect-video object-cover rounded-lg transition-transform duration-300 border dark:border-none"
            src={post.thumbnail}
            alt={post.title}
            width={280}
            height={158}
