      ...
      <div className="mt-4 mb-0 flex flex-wrap gap-2">
        {post.tags.map((tag) => (
          <Link href={`/tags/${tag}`} key={tag} className="no-underline">
            <p className="mt-0 mb-0 text-sm bg-gray-100 text-gray-500 dark:text-gray-700 dark:bg-gray-300 rounded-full px-2.5 py-2 hover:bg-gray-200 dark:hover:bg-gray-400 transition-colors">
              #{tag}
            </p>
          </Link>
        ))}
